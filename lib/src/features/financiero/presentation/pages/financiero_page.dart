...
class _FinancieroViewState extends State<_FinancieroView>
  ...
  void _confirmarDesactivar(BuildContext ctx, TarifaModel t) =>
      _confirmDialog(ctx,
        ...
        labelOk: 'Desactivar',
        colorOk: const Color(0xFFC62828),
  ...
  void _confirmarAprobar(BuildContext ctx, CotizacionModel c) =>
      _confirmDialog(ctx,
        ...
        labelOk: 'Aprobar',
        colorOk: const Color(0xFF2E7D32),
  ...
  void _confirmarEmitir(BuildContext ctx, FacturaModel f) =>
      _confirmDialog(ctx,
        ...
        labelOk: 'Emitir',
        colorOk: const Color(0xFF1A237E),
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          TextField(
            controller: ctrl,
            decoration: _deco('Motivo de anulación *'),
            maxLines: 2,
          ),
        ]),
        actions: [
            ...
            children: [
              ...
              ...items.map((t) => TableRow(
                ...
                children: [
                  ...
                  _td(Text(_fmtMonto(t.precioBase),
                      style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13))),
                  _td(Text(t.precioPorLibra != null ? _fmtMonto(t.precioPorLibra!) : '—',
...
class _CotizacionesTab extends StatelessWidget {
  ...
  const _CotizacionesTab({required this.onAdd, required this.onAprobar, required this.onCancelar});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<FinancieroBloc, FinancieroState>(
      builder: (ctx, state) {
        ...
        final isDesktop = MediaQuery.of(context).size.width >= 900;
        return isDesktop
            ? _CotizacionesDesktop(items: lista, onAprobar: onAprobar, onCancelar: onCancelar)
            : _CotizacionesMobile(items: lista, onAprobar: onAprobar, onCancelar: onCancelar);
      },
    );
  }
}
...
class _CotizacionesDesktop extends StatelessWidget {
  ...
  final void Function(CotizacionModel) onAprobar;
  final void Function(CotizacionModel) onCancelar;
  const _CotizacionesDesktop({required this.items, required this.onAprobar, required this.onCancelar});
...
class _CotizacionesMobile extends StatelessWidget {
  ...
  final void Function(CotizacionModel) onAprobar;
  final void Function(CotizacionModel) onCancelar;
  const _CotizacionesMobile({required this.items, required this.onAprobar, required this.onCancelar});
            ...
            children: [
              ...
              ...items.map((f) => TableRow(
                decoration: const BoxDecoration(
                    border: Border(top: BorderSide(color: Color(0xFFE5E7EB)))),
                children: [
                  _td(Text(f.numeroFactura ?? 'BORRADOR',
                      style: TextStyle(
                          fontWeight: FontWeight.w600, fontSize: 12,
                          color: f.numeroFactura != null
                              ? const Color(0xFF1A237E) : const Color(0xFF9CA3AF)))),
                  _td(Text(f.clienteNombre ?? '—',
                      style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500))),
                  _td(Text(_fmtFecha(f.fechaEmision),
                      style: const TextStyle(fontSize: 12, color: Color(0xFF6B7280)))),
                  _td(Text(_fmtMonto(f.iva),
                      style: const TextStyle(fontSize: 12, color: Color(0xFF6B7280)))),
                  _td(Text(_fmtMonto(f.total),
                      style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13,
                          color: Color(0xFF1A237E)))),
            ...
            children: [
              ...
              ...items.map((p) => TableRow(
                decoration: const BoxDecoration(
                    border: Border(top: BorderSide(color: Color(0xFFE5E7EB)))),
                children: [
                  _td(Text(p.numeroPago,
                      style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 12,
                          color: Color(0xFF1A237E)))),
                  _td(Text(p.clienteNombre ?? '—',
                      style: const TextStyle(fontSize: 13))),
                  _td(Text(p.facturaNumero ?? p.facturaId.substring(0,8),
                      style: const TextStyle(fontSize: 12, color: Color(0xFF6B7280)))),
                  _td(Text(_fmtMonto(p.monto),
                      style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13,
                          color: Color(0xFF1A237E)))),
...
// ═══════════════════════════════════════════════════════════════════════════════
// DETALLE DE FACTURA (bottom sheet)
// ═══════════════════════════════════════════════════════════════════════════════
    ...
    return BlocBuilder<FinancieroBloc, FinancieroState>(
      builder: (ctx, state) {
        if (state is FinancieroLoading) {
          return _Sheet(title: 'Detalle de factura', child:
...
class _TarifaFormSheetState extends State<_TarifaFormSheet> {
  ...
  void _submit() {
    ...
    final data = <String, dynamic>{
      ...
      'categoria':    _cat.name,
      'tipoPedido':   _tipo.name,
      'precioBase':   double.tryParse(_precioBaseC.text) ?? 0,
      'porcentajeIva':double.tryParse(_ivaC.text) ?? 15.0,
      'activo':       _activo,
    ...
    child: Form(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        DropdownButtonFormField<CategoriaPaquete>(
          value: _cat,
          decoration: _deco(''),
          items: CategoriaPaquete.values
              .map((c) => DropdownMenuItem(value: c, child: Text(c.label))).toList(),
          onChanged: (v) => setState(() => _cat = v!),
        ),
        const SizedBox(height: 14),
        const _Label('Tipo de pedido *'),
        const SizedBox(height: 8),
        DropdownButtonFormField<TipoPedidoFinanciero>(
          value: _tipo,
          decoration: _deco(''),
          items: TipoPedidoFinanciero.values
        ...
        Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const _Label('Precio base (USD) *'),
            const SizedBox(height: 8),
            TextFormField(
              controller: _precioBaseC,
          ...
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const _Label('Por libra (USD)'),
            const SizedBox(height: 8),
            TextFormField(
              controller: _porLibraC,
              decoration: _deco('0.00'),
              keyboardType: const TextInputType.numberWithOptions(decimal: true),
            ),
          ])),
        ...
        Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const _Label('Peso mínimo (lb)'),
            const SizedBox(height: 8),
            TextFormField(
              controller: _pesoMinC,
              decoration: _deco('0.0'),
              keyboardType: const TextInputType.numberWithOptions(decimal: true),
            ),
          ])),
          const SizedBox(width: 12),
        ...
        _SheetBtn(
          label: _isEdit ? 'Guardar cambios' : 'Crear tarifa',
...
class _CotizacionFormSheetState extends State<_CotizacionFormSheet> {
  ...
  @override
  void dispose() {
    for (final c in [_clienteIdC, _pedidoIdC, _pesoC, _largoC, _anchoC, _altoC, _valorC, _obsC])
      c.dispose();
    super.dispose();
  }
  ...
  void _submit() {
    if (!_key.currentState!.validate()) return;
    final data = <String, dynamic>{
      'clienteId': _clienteIdC.text.trim(),
      'categoria': _cat.name,
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Container(
          ...
          child: const Row(children: [
            ...
            SizedBox(width: 8),
            Expanded(child: Text(
                'Si ingresas el ID de una cotización aprobada, los montos se cargarán automáticamente.',
        ...
        TextFormField(controller: _clienteIdC, decoration: _deco('UUID del cliente'),
            validator: (v) => v == null || v.isEmpty ? 'Requerido' : null),
        const SizedBox(height: 14),
        const _Label('ID de la cotización (opcional)'),
        const SizedBox(height: 8),
        ...
        TextFormField(controller: _pedidoIdC, decoration: _deco('UUID del pedido')),
        const SizedBox(height: 14),
        const _Label('Forma de pago *'),
        const SizedBox(height: 8),
        DropdownButtonFormField<FormaPago>(
          value: _formaPago,
          decoration: _deco(''),
          items: FormaPago.values
  ...
  Widget build(BuildContext context) => _Sheet(
    ...
    child: Form(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        const SizedBox(height: 14),
        const _Label('Referencia (transferencia/voucher)'),
        const SizedBox(height: 8),
...
// ═══════════════════════════════════════════════════════════════════════════════
// WIDGETS AUXILIARES (misma estructura que clientes_page.dart)
// ═══════════════════════════════════════════════════════════════════════════════
...
class _ActiveBadge extends StatelessWidget {
  final bool activo;
  const _ActiveBadge(this.activo);
  @override
...
class _SheetBtn extends StatelessWidget {
  ...
  final VoidCallback onTap;
  final bool      outlined;
  const _SheetBtn({required this.label, this.icon,
    this.color = const Color(0xFF1A237E), required this.onTap, this.outlined = false});
  @override
  Widget build(BuildContext context) {
...
class _MontoRow extends StatelessWidget {
  ...
  const _MontoRow(this.label, this.value,
      {this.isBold = false, this.isTotal = false, this.isNegative = false});
  @override
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(children: [
      ...
      Text('${isNegative ? '-' : ''}${_fmtMonto(value)}', style: TextStyle(
          fontSize: isTotal ? 15 : 13,
          fontWeight: isBold ? FontWeight.bold : FontWeight.normal,
          color: isTotal ? const Color(0xFF1A237E) :
          isNegative ? const Color(0xFFC62828) : const Color(0xFF374151))),
    ]),
  );
}
